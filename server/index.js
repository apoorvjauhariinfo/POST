...
app.get(
  "/inventorymanagerbyhospitalid/:hospitalid",
...
//Get Admin by ID
app.get("/adminbyid/:id", verifyToken(["admin"]), async (req, res) => {
  const { id } = req.params;
  // checking valid user
  const { middleware_role, middleware_id } = req.body;
  if (middleware_role === "admin" && middleware_id != id) {
...
app.get(
  ...
  async (req, res) => {
    ...
    // checking valid user
    const { middleware_role, middleware_id } = req.body;
    if (middleware_role === "inventorymanager" && middleware_id != id) {
      return res.status(403).json({ message: "Forbidden" });
    }
    try {
...
app.get(
  "/productbyhospitalid/:hospitalid",
  verifyToken(["admin", "user", "inventorymanager"]),
  async (req, res) => {
    const { hospitalid } = req.params;
...
app.get(
  "/productcountbyhospitalid/:hospitalid",
  verifyToken(["admin", "user", "inventorymanager"]),
  async (req, res) => {
    const { hospitalid } = req.params;
...
app.get(
  "/historywithproductdetails/:hospitalid",
  verifyToken(["admin", "user", "inventorymanager"]),
  async (req, res) => {
    const { hospitalid } = req.params;
...
app.put("/updateadmin/:id", verifyToken(["admin"]), async (req, res) => {
  try {
    const { id } = req.params;
    const { password, status } = req.body;
    // checking valid user
...
app.delete("/deleteadmin/:id", verifyToken(["admin"]), async (req, res) => {
  try {
    const { id } = req.params;
    // checking valid user
...
app.post("/users", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      "dsfnsjfjsf", // Use an environment variable for the secret
      { expiresIn: "7d" }
    );
    ...
    return res.status(200).json({ message: "Login successful", user: detail });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
app.get(
  "/stocks/outvalue/details/hospital/:hospitalid",
  verifyToken(["admin", "user", "inventorymanager"]),
  async (req, res) => {
    const { hospitalid } = req.params;
...
app.post("/inventorymanagers", async (req, res) => {
  // //const { walletAddress } = req.params;
  // const udoc = await InventoryManager.find();
  // console.log(udoc)
  // res.json({ document });
  const { email, password } = req.body;
  ...
  try {
    const user = await InventoryManager.findOne({ email: email });
    console.log(user);
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
